...
function installLocaleRefresh(window: BrowserWindow): void {
  // Electron has no dedicated OS-language event. Check whenever any 鸿翼灵工
...
// single-worktree case still lands at "鸿翼灵工 Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `鸿翼灵工 Canary ${process.env.DESKTOP_APP_SUFFIX}`
  : "鸿翼灵工 Canary";
...
if (is.dev) {
  ...
  // to "鸿翼灵工", but anchoring it here makes WM_CLASS ↔ StartupWMClass
