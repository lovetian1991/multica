...
function rendererRecoveryDetail(payload: ReloadPromptPayload) {
  const guidance = [
    "Click Reload to refresh this window and keep using 鸿翼灵工.",
  ...
  if (payload.kind === "unresponsive") {
    guidance.push(
      "For macOS reports, an Activity Monitor sample of the 鸿翼灵工 Helper (Renderer) process helps us find what blocked the app.",
    );
  }
