...
const en = `Welcome to 鸿翼灵工.

Agents need a runtime before they can execute work. You can still use 鸿翼灵工 as a lightweight project-management workspace while you install one.
...
4. Confirm your terminal can find it:
   which codex
   codex --version
5. Wait for 鸿翼灵工 to pick it up. A running daemon re-checks for newly
...
智能体需要先连上运行时才能执行工作。运行时还没准备好时,你也可以先把 鸿翼灵工 当作轻量项目管理工具体验起来。
...
5. 等 鸿翼灵工 识别到它。运行中的守护进程每隔几分钟会重新检查一次新装的 CLI,通常不需要重启。
   想立刻生效:
   multica daemon restart
...
const ko = `鸿翼灵工에 오신 것을 환영합니다.

agent가 작업을 실행하려면 먼저 runtime이 필요합니다. runtime을 설치하는 동안에도 鸿翼灵工를 가벼운 프로젝트 관리 워크스페이스로 먼저 사용할 수 있습니다.
...
4. 터미널에서 찾을 수 있는지 확인합니다:
   which codex
   codex --version
5. 鸿翼灵工가 인식할 때까지 기다립니다. 실행 중인 daemon은 몇 분마다 새로 설치된 CLI를
...
agent が作業を実行するには、まず runtime が必要です。runtime をインストールしている間も、鸿翼灵工 を軽量なプロジェクト管理ワークスペースとして先に使うことができます。
...
4. ターミナルから見つけられるか確認します:
   which codex
   codex --version
5. 鸿翼灵工 が認識するまで待ちます。動作中の daemon は数分ごとに新しくインストールされた
